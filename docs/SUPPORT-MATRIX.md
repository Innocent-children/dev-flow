...
| `dev-flow-deepseek` | private source package; public version not selected | exact DSH `0.1.0-rc.8`, macOS arm64 | Simplification CI and repeatable Preflight passed; the single Native Acceptance timed out at `design-and-tasks` after Core reached `IMPLEMENT` revision 4; final acceptance remains blocked and not publicly supported | `specs/010-deepseek-explicit-graph-host/` |
