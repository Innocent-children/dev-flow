...
| `dev-flow-deepseek` | `0.5.0` publishable source package | DSH `>=0.1.0-rc.6`, macOS arm64 | Source-local acceptance complete; unpublished and not publicly supported until the independent release gate passes | `packages/deepseek/package.json`, `tests/journeys/deepseek/evidence/native-acceptance.json` |
