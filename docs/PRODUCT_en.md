...
On every entry to understanding review, the current Task AI proactively reviews existing requirements, design, code changes, discussion and checks. It extracts a few useful experiences, saves them through Core, then explains the reasons, applicability and next checks. No separate collection request is needed. Nothing useful means no write; collection adds no investigation or tests. Other nodes do not require automatic collection.

Users can keep asking questions and save actual supplements. Changed conclusions and later reviews after code changes update the matching experience rather than duplicating it. Failed saves are reported and resolved, never treated as no experience or confirmed understanding. Delivery still requires the user's explicit understanding verdict. Core stores experience independently of Task revision; collection is performed by the current AI without chat monitoring or another model. Task completion automatically attempts Markdown export outside repositories; failure preserves completion and supports retry. WebUI supports later reading, user supplements, project and keyword lookup, including archived tasks. Historical reading and manual export remain available. See [Task experiences](EXPERIENCES_en.md).
