...
| 产品 | npm `beta` | `main` 中的 package 版本 | 当前新增能力 |
| --- | --- | --- | --- |
| `dev-flow-codex` | `0.6.0-beta.2` | `0.6.0-beta.2` | 智能选择、改进后的 setup、多仓库 Task Scope |
| `dev-flow-deepseek` | `0.6.0-beta.1` | `0.6.0-beta.1` | 多仓库 Task Scope |

beta 可以公开安装，但它不是稳定支持声明。源码可构建、Feature Journey 通过或 beta 已发布，都不等于
该能力已经进入 npm `@latest`。
...
- 当前没有 Web UI、遥测、用户自定义流程图或自动历史 Task 迁移。
