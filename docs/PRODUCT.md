...
Codex Host 在每次普通提交和一次允许的修正提交前，按当前 `submission_tool` 的实时 schema 逐项核对
完整草稿，包括每层必填与额外成员、嵌套值和数组元素类型、nullability、enum 与 const。草稿无法
精确匹配时会在调用工具前停止；字段名、参考说明和错误文本都不能代替实时 schema 定义类型。
...
共享页面框架会在桌面、平板和窄屏间重排导航、筛选、Task 列表、详情、表单和系统状态：宽屏扩大结构化内容区，窄屏把任务表格改为带字段名的卡片，同时保持相同路由和操作语义。
