...
> Dev Flow 是长时 AI 编程任务的本地过程控制与恢复层。

它在聊天记录之外保存任务目标、范围、当前阶段、验证预算、已有记录、阻塞原因和恢复状态，让
Codex 或 DeepSeek 在会话压缩、Host 重启或操作结果不确定后，能够继续同一个任务，而不是重新
推测进度。
...
3. 代码已经变化，旧验证记录却继续被当作当前结果；
4. 写操作响应丢失后被直接重复执行；
5. 测试通过，但实现仍然过度复杂或难以解释。

## 产品如何介入

Codex 或 DeepSeek 仍然读取仓库、修改文件和运行命令。Dev Flow 在这些工作之外维护一个本地 Task：

- 创建时保存请求、验收条件、范围和 verification budget；
- 每个阶段返回完成条件、允许的操作和可选下一步；
- 记录实现、测试、理解确认、阻塞和交付结果；
- 在仓库状态漂移或当前条件不满足时阻塞流转；
- 在 Action 结果不确定时执行 read-before-retry，并给出 Recovery 判断。
...
保存最初请求、明确范围和 verification budget；上游要求或实现发生变化时，让不再适用的下游记录
失效。

### 第三层：中断与不确定操作恢复

保存 Action 身份和恢复所需状态；响应丢失、取消或截断时，先读取当前状态，再选择继续、恢复、
阻塞或安全重试。
