...
| Context menu / menu bar | Choose tasks and appearances, import packs, control animations and idle activities, hide, or quit. |
...
| --- | --- |
| Single PNG | All five task clips share one image, producing a static appearance. |
| Native Dev Flow animation pack | Five task clips are required and four additional clips are optional; each clip's frame count comes from its own catalog. |
...
| Walk right, walk left | Initial weight: 25% each. With at least 40 pt available in the direction, move horizontally 40–80 pt at about 20 pt/second for 2–4 seconds. A finished walk has a 30% chance of continuing with one wave. |
...
- Dev Flow PNG animation frame dimensions must match `canvas`, whose width and height are positive integers. Static images and individual PNG frames
  have an estimated decoding limit of 128 MiB each.
- Catalogs contain at most 512 frame references. Referenced PNG files total at most 128 MiB to accommodate nine high-resolution clips. Each clip's RGBA data, estimated as
...
| Must every pack contain nine clips and 57 frames? | This fixed count applies only to Codex-layout atlases. Single PNGs and native animation packs follow their own rules in the table above. |
