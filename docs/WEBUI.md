...
执行任务的 AI 在每次进入理解确认时主动回顾已有工作，保存有价值的经验后讲解；没有值得记录的内容可以跳过。
再次进入时更新已有经验，用户确认理解后才进入交付。WebUI 展示已保存内容，用户补充、历史查阅和手动导出
不要求任务停留在理解确认节点。

“任务经验”页面按关键词和项目查找当前结论与用户补充，包括已归档任务。任务详情展示原因、依据、适用条件、引用和修订历史，支持用户补充与 Markdown 导出。导出失败可以直接重试，无需恢复代码任务。详见[任务经验](EXPERIENCES.md)。
