...
- Os envios de Design, Tasks e Implementation omitem, respectivamente, `requirements_revision`, `design_revision` e `task_plan_revision`; depois de validar a identidade da Action atual, Core preenche esses campos a partir da mesma Task snapshot. Antes de registrar, Core também valida a semântica do resultado contra a Task atual. Erros em revision, record, conjuntos de evidence e acceptance que podem ser copiados exatamente de Core retornam `allowed_paths`. Um `required_member_missing` com zero escrita comprovada em um envio de nó também pode ser corrigido uma vez no caminho exato, usando apenas fatos já confirmados no trabalho do nó atual. Se o conteúdo ausente exigir uma nova decisão do usuário, o Host deve parar e solicitá-la; outros valores que não possam ser derivados com segurança não autorizam correção automática.
