...
**タスクの経験を残す：** 理解確認に入るたびに、タスクを実行する AI が既存の作業を自動で振り返り、有用な知見を少数保存してから、理由、適用条件、次回の確認点を説明します。残す価値のある内容がなければ記録を省略し、再び理解確認に入ったときは既存の知見を更新します。ユーザーによる明示的な理解確認は引き続き必要です。完了時にはワークツリーの外に Markdown を出力します。WebUI で後から読み返し、プロジェクトやキーワードで検索し、自分の理解を追記して、出力を再試行できます。[タスクの経験](docs/EXPERIENCES_en.md)を参照してください。
