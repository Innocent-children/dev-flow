...
macOS arm64 と Windows 10/11 x64 向けのローカル開発パッケージには、デスクトップペットがあります。選択した Task の保存済み状態を表示し、対応する WebUI を開きます。Host のリアルタイム動作や完了率を示すものではありません。タスク選択、外観の追加、アニメーションの切り替え、サイズ変更、個別の起動と終了に対応します。Core はインストール・設定済みの Codex または DeepSeek Adapter が提供します。

通常の npm パッケージには macOS のネイティブアプリは含まれません。ローカルパッケージの取得、ビルド、インストール、更新、素材については[デスクトップペットガイド](docs/DESKTOP-PETS_en.md)を参照してください。ローカル検証によって[安定版のサポート範囲](docs/SUPPORT-MATRIX_en.md)が広がることはありません。
