...
- **不確実な書き込みの復旧：** Core が正規化済み Action 入力を保持するため、応答喪失後は Task ID と Action ID だけで復旧でき、payload の再構築は不要です。
...
- Core は Action 提出を保存する前に、現在の Task に対してノード結果の意味も検証します。revision、record、evidence 集合、acceptance など Core から正確にコピーできる値の誤りは `allowed_paths` を返し、Host は一度だけ限定修正できます。テスト結論、ユーザー確認、作業内容など安全に導出できない値はフィールド情報だけを返し、自動修正を許可しません。
