...
- **worktree ごとに変更の所有者を固定します。** Core は専用 worktree の Git 状態から現在の Task
  surface を計算します。通常の線形 commit は継続でき、branch rewrite や worktree の置き換えは停止します。
...
セッションの再起動後は、Task に結び付いた元の worktree に戻り、明示的に再開を依頼します。
再開時に准入評価や代替 worktree の選択は行いません。元のインスタンスが消失または置換されている
場合は、復元するか明示的に abandon するまで停止します。
