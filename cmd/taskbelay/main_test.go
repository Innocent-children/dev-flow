...
func TestRunHelp(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assertContainsAll(t, stdout.String(), "governed Core", "local STDIO MCP", "taskbelay mcp --stdio", dataDirectoryEnvironment)
...
func TestRunVersionUsesCurrentRepositoryVersion(t *testing.T) {
	...
	if stderr.Len() != 0 || stdout.String() != "taskbelay "+wantVersion+"\n" {
...
func TestRunMCPStdioStartsAndStopsCleanlyOnEOF(t *testing.T) {
	...
	configDirectory := filepath.Join(homeDirectory, ".taskbelay")
...
func TestRunRejectsEveryOtherCommandAndNetworkMode(t *testing.T) {
	...
	for _, args := range tests {
		...
		if stdout.Len() != 0 || stderr.String() != "taskbelay: invalid arguments; use \"taskbelay help\"\n" {
...
func TestRunWebUIFullPublicLifecycle(t *testing.T) {
	...
	state := webui.RuntimeState{Readiness: webui.ReadinessReady, CoreIdentity: "taskbelay/test", DataRootDigest: strings.Repeat("a", 64), URL: "http://127.0.0.1:1234", PID: 42}
