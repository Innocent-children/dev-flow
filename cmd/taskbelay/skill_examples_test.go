...
func validateSkillArtifactExamples(t *testing.T, host string) {
	path := filepath.Join("..", "..", "packages", "codex", "plugin", "skills", "taskbelay", "references", "artifact-contract.md")
	if host == "deepseek" {
		path = filepath.Join("..", "..", "packages", "deepseek", "skills", "taskbelay", "references", "artifact-contract.md")
	}
	if host == "claude" {
		path = filepath.Join("..", "..", "packages", "claude", "plugin", "skills", "taskbelay", "references", "artifact-contract.md")
	}
	if host == "zcode" {
		path = filepath.Join("..", "..", "packages", "zcode", "skills", "taskbelay", "references", "artifact-contract.md")
	}
