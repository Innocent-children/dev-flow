...
func runWorkspaceAvailabilityCheck(stdin io.Reader, stdout, stderr io.Writer, getenv func(string) string) int {
	...
	if decoder.Decode(&input) != nil || decoder.Decode(&trailing) != io.EOF {
		fmt.Fprintln(stderr, "taskbelay: workspace availability input is invalid")
		return 1
	}
	dataDirectory := getenv(dataDirectoryEnvironment)
	if !usableDataDirectory(dataDirectory) {
		fmt.Fprintln(stderr, "taskbelay: workspace availability data directory is unavailable")
		return 1
	}
	...
	if _, err := os.Stat(databasePath); err == nil {
		...
		if openErr != nil {
			fmt.Fprintln(stderr, "taskbelay: workspace availability storage is unavailable")
			return 1
		}
		defer tasks.Close()
		lookup = tasks
	...
	if err != nil {
		fmt.Fprintln(stderr, "taskbelay: workspace availability check failed")
		return 1
	}
