...
Dev Flow는 합의한 요청, 예정 경로, 검증 한도, 현재 단계, 결과를 하나의 로컬 작업으로 저장합니다.
코드 변경은 계속 Codex 또는 DeepSeek가 수행합니다.

- **범위를 명확하게 유지합니다.** 예정 경로를 기록하고, 지원되는 구조화 도구가 계획 밖 파일에 쓰기
  전에 확인하며, 테스트와 전달 전에 실제 변경을 다시 대조합니다.
- **검증량에 한도를 둡니다.** 자동 확인의 명령 수를 제한하고, 전체 스위트에는 사전 허용을 요구하며,
  세 번째 정확한 반복에서 작업을 멈춥니다.
...
세션이 재시작되면 같은 저장소 작업 디렉터리로 돌아가 동일한 selector를 다시 사용하세요. Dev Flow는
저장된 작업을 읽고 현재 단계부터 이어갑니다.
...
- **프로젝트:** [Product](docs/PRODUCT_en.md) · [Support Matrix](docs/SUPPORT-MATRIX_en.md) · [Security](SECURITY.md) · [Contributing](CONTRIBUTING_en.md)
