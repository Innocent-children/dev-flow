...
- **Tasks paralelas en un mismo repositorio:** un repositorio Git lógico puede ejecutar varias Tasks independientes al mismo tiempo mediante linked worktrees. Cada worktree físico conserva como máximo una Task activa. Codex solo distribuye automáticamente cuando el Host ya ofrece una capacidad task/thread respaldada por worktrees; de lo contrario pide iniciar otro worktree. Core no crea, cambia ni limpia worktrees.
...
- Los envíos de Design, Tasks e Implementation omiten, respectivamente, `requirements_revision`, `design_revision` y `task_plan_revision`; después de validar la identidad de la Action actual, Core completa esos campos desde la misma Task snapshot. Antes de guardar, Core también valida la semántica del resultado contra la Task actual. Los errores en revision, record, conjuntos de evidence y acceptance que pueden copiarse exactamente desde Core devuelven `allowed_paths`. Un `required_member_missing` con cero escrituras demostrado en un envío de nodo también puede corregirse una vez en su ruta exacta, usando solo hechos ya confirmados durante el trabajo del nodo actual. Si falta una nueva decisión del usuario, el Host debe detenerse y solicitarla; los demás valores que no puedan derivarse con seguridad no autorizan corrección automática.
