...
Use the [Host command transport](transport.md) for each JSON object below. Paths, IDs and digests are examples. Keep the actual `inspect` response as `assessment.anchor`, the actual `prepare` response's `launch_id` for subsequent calls, and the exact `scope` result for `dev_flow_open_task`. Do not reuse sample identities.
...
```json
{
  ...
  "user_choice": {"source":"user","mode":"dev_flow","summary":"Use Dev Flow in a new local branch without carrying changes."},
