...
Implementation: `packages/zcode/hooks/pre-tool-use.mjs`, `cmd/dev-flow/artifacts.go` and `internal/application/file_scope.go`.

Before each submission use `dev-flow-zcode artifacts collect` and `prepare` according to the [shared artifact contract](artifact-contract.md). Supply current `host=zcode`, Task and Action. Preserve all observed files and their identities. Classify only the slot and summary; never invent paths or digests.
