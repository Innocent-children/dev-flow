...
The receipt binds request to assessment and handoff to its content digest. A changed request or discussion is rejected before preparation is consumed; inspect the original saved record rather than creating another launch. The receipt is retained outside Task workspaces under the Dev Flow product directory, in `provisioning/zcode/<launch_id>/receipt.json`.
