...
export async function ensureUserConfiguration(paths) {
  ...
  await ensureConfigurationDirectory(configurationDirectory);
  try {
  ...
  await validateExistingConfiguration(configurationPath);
...
async function ensureConfigurationDirectory(path) {
  try {
...
async function validateExistingConfiguration(path) {
  let info;
  try {
