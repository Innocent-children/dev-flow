...
`tools.mcp__taskbelay__taskbelay_get_task`. The Core example's JSON is the complete tool argument;
Core tool names are not shell commands. Set `host="codex"` except for `taskbelay_server_info`,
whose complete argument is `{}`.

```js
const response = await tools.mcp__taskbelay__taskbelay_get_task({host: "codex", task_id: current_task_id});
...
order; `store` is a Host convenience, not a TaskBelay API or a required production dependency.
