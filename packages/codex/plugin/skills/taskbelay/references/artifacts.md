...
Implementation: `packages/codex/bin/taskbelay-codex.mjs` — `runCLI`.
Implementation: `cmd/taskbelay/artifacts.go` — `runArtifacts`.
...
taskbelay-codex artifacts collect < /private/tmp/collect-input.json > /private/tmp/collect-output.json
```
...
Implementation: `cmd/taskbelay/main.go` — `runPreFileWriteCheck`.

The installed `hooks/hooks.json` matches apply_patch and invokes `taskbelay-codex hook pre-tool-use`.
...
`taskbelay-codex host-check pre-file-write` entry with this shape:
