...
test("DeepSeek owns the current receipt format and creates its directory only when writing", async t => {
  const home = await realpath(await mkdtemp(join(tmpdir(), "dev-flow-profile-records-")));
  ...
  const receipt = { profile: "web", package_name: "dev-flow-deepseek", installed_version: "0.8.0", origin: "installed",
  ...
  for (const invalid of [{ ...receipt, extra: true }, { ...receipt, package_name: "dev-flow-codex" }, { ...receipt, installed_version: "latest" }]) {
...
test("DeepSeek does not follow a linked Profile record directory", async t => {
  const root = await realpath(await mkdtemp(join(tmpdir(), "dev-flow-profile-link-")));
