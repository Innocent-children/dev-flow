  ...
  test(`DeepSeek installs a verified Profile from npm pack's ${format} report`, async (t) => {
    ...
    const run = async (executable, arguments_, options = {}) => {
      ...
      if (executable === "npm" && arguments_[0] === "pack") {
        ...
        const report = { name: "dev-flow-deepseek", version: "0.8.0", filename };
        return { stdout: `${JSON.stringify(format === "array" ? [report] : { "dev-flow-deepseek": report })}\n`, stderr: "" };
      }
      return { stdout: "", stderr: "" };
    };
...
test("DeepSeek rejects ambiguous or mismatched npm reports before changing the Profile", async t => {
  const valid = { name: "dev-flow-deepseek", version: "0.8.0", filename: "dev-flow-deepseek-0.8.0.tgz" };
  for (const [name, report] of [
    ["wrong package", { "dev-flow-deepseek": { ...valid, name: "another-package" } }],
    ["wrong version", { "dev-flow-deepseek": { ...valid, version: "0.7.0" } }],
    ["outside download directory", { "dev-flow-deepseek": { ...valid, filename: "../artifact.tgz" } }],
    ["multiple packages", { "dev-flow-deepseek": valid, "another-package": valid }],
    ["no packages", {}],
  ]) {
