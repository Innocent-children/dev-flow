...
const packageName = "dev-flow-zcode";
const stable = value => {
  ...
  return {
    ...
    async observe() {
      ...
      try {
        ...
        } catch (failure) { issues.push({ code: "package_check_failed", message: failure.message, command: "dev-flow doctor --host zcode" }); }
        if (packageVersion) issues.push({ code: "adapter_check_failed", message: error.message, command: "dev-flow repair --host zcode --yes" });
      }
      try { receipt = await readReceipt(); }
      catch (error) { issues.push({ code: "registration_check_failed", message: error.message, command: "dev-flow doctor --host zcode" }); }
      if (report?.status === "partial") issues.push({ code: "adapter_check_failed",
        message: report.issues?.join("; ") || "The local ZCode package needs preparation or repair.", command: "dev-flow repair --host zcode --yes" });
      ...
      if (receipt || report?.status === "action_required") {
        ...
        issues.push({ code: "adapter_missing", message: "Dev Flow ZCode Adapter is not installed.", command: "dev-flow install --host zcode --yes" });
      }
        ...
        if (operation === "uninstall") {
          ...
          if (observed.receipt) {
            ...
            return { changed: removed.changed, completedSteps, nextSteps: [
              ...removed.next_steps,
              "After confirming Host removal, run dev-flow uninstall --host zcode --yes to uninstall the Adapter package.",
            ] };
          }
