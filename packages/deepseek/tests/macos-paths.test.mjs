...
describe("macOS arm64 paths and POSIX runtime checks", {
  skip: process.platform === "darwin" && process.arch === "arm64" ? false : "requires macOS arm64",
}, () => {
  test("detached package loads all platform modules and preflights a macOS shell fixture", async (t) => {
    ...
    for (const file of ["paths.mjs", "platform.mjs", "runtime.mjs", "platform/macos/policies.mjs", "platform/windows/policies.mjs"]) {
      const target = join(detachedRoot, "lib", file);
