...
Each call requires /taskbelay in the current direct user turn. Example: “/taskbelay cancel Task
...
```text
/taskbelay prepare-cleanup launch=11111111-1111-4111-8111-111111111111 repository=primary task=task-example revision=9
```
...
```text
/taskbelay cleanup-worktree launch=11111111-1111-4111-8111-111111111111 repository=primary task=task-example revision=9
```
...
```text
/taskbelay cleanup-branch launch=11111111-1111-4111-8111-111111111111 repository=primary task=task-example revision=9
```
...
A missing exact confirmation returns TASKBELAY_WORKSPACE_CLEANUP_CONFIRMATION_REQUIRED. Ask for the
...
`packages/taskbelay/lib/hosts/deepseek.mjs`.

This package is a DSH bundle with no standalone taskbelay-deepseek CLI. Use the actual Profile and the
...
```sh
dsh plugin --profile web remove taskbelay-deepseek
dsh --profile web --dump-config
```
...
supported platform runtime, checks it and registers Skill/MCP/guards. TASKBELAY_DATA_DIR must be set
