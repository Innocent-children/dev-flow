...
`packages/deepseek/lib/tool-names.mjs` — `TASKBELAY_QUALIFIED_TOOL_NAMES`;
`packages/deepseek/lib/workspace-tool.mjs` — `readCoreTask`.

## Names, arguments and authorization

Call the visible DSH MCP tool whose name is `mcp__taskbelay__` plus the Core raw name. For example,
Core returns submission_tool taskbelay_submit_test; invoke mcp__taskbelay__taskbelay_submit_test with
the complete TEST input. The common examples' marker/tool field remains a Core raw name; the DSH
call name is qualified. Use `host:"deepseek"` except for `taskbelay_server_info`, whose complete
argument is `{}`. The current-turn selector is still required for the handshake.

For a current direct user message such as “/taskbelay continue Task task-example”, use the complete [saved Task read request and response](connection.md#read-saved-state-and-the-next-action),
invoked as `mcp__taskbelay__taskbelay_get_task` with `host:"deepseek"`.
...
call, including nested calls. TASKBELAY_SELECTOR_REQUIRED/NO_OPEN_TURN/NO_AGENT stops the call; ask
...
Implementation: `packages/deepseek/lib/authorization.mjs` — `authorizeTaskBelayExecution`, `deriveCurrentTurn`.

## Complete result handling

The integration configures the DSH MCP client to the packaged stdio Core. Read the complete TaskBelay
...
The [complete successful Task read](successes/taskbelay_get_task-read.md) includes the resolved
