...
`/dev-flow` in the current direct user message.** Earlier turns, assistant messages and Skill injection
...
| --- | --- |
| New development request, including `/dev-flow` | [Assess and prepare](references/admission.md); default to a new branch in the current directory after Dev Flow selection. |
...
visible `mcp__dev_flow__` tool plus Core's raw name through the DSH transport. The handshake takes
