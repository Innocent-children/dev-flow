<!-- Generated from skills/taskbelay/core/method-profiles.md; edit the shared source and run node scripts/sync-skill-references.mjs. -->
...
| `test.classify_failure` | Distinguish new from known failures; compare known failures and obtain explicit acceptance of their exact set when appropriate. | Route failures that require changes by their implementation, design, or requirement cause. When accepting known failures, retain the comparison and the user's explicit acceptance of the exact failed set. | Direct classification, comparison, and user interaction; no mandatory capability. | Direct classification, comparison, and user interaction; no mandatory capability. | Failure classification and findings; when accepting known failures, a passed automated comparison and the user's acceptance tied to the current content and Task Plan. |
