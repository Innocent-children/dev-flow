...
Use the [Host command transport](transport.md) for each JSON object below. The paths, IDs and digests are examples. Copy the actual `inspect` result into `assessment.anchor`, the actual `prepare` result's `launch_id` into later inputs, and the exact `scope` result into the repository members of `dev_flow_open_task`. Do not recompute or reuse example identities.
