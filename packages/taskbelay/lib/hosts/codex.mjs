...
export const CODEX_ACTIVATION_STEP = "Open Codex /hooks, review and trust the TaskBelay hook, then start a new conversation.";
...
export function createCodexDriver({
  ...
  adapterExecutable = "taskbelay-codex",
  localPackage = null,
  paths = null,
} = {}) {
  return Object.freeze({
    async runtimeCandidates() {
      ...
      if (product.name !== "taskbelay-codex") throw new Error("Codex receipt product identity is invalid");
      ...
      return [{ source: "codex", packageName: "taskbelay-codex", packageVersion: product.version,
    ...
    async maintenanceTargets({ observed }) {
      ...
      if (observed.packageInstalled) {
        ...
        installedRuntime = await installedCoreRuntime(join(root, "taskbelay-codex"), paths,
          { host: "codex", profile: null, packageName: "taskbelay-codex" });
      }
    ...
    async observe() {
      ...
      catch (error) { issues.push(issue("host_check_failed", error, "taskbelay doctor --host codex")); }
      ...
      try {
        ...
        issues.push(issue("adapter_check_failed", error, "taskbelay repair --host codex --yes"));
      }
      ...
      if (!status.available || issues.some(entry => entry.code === "adapter_check_failed")) {
        try {
          let listed;
          try {
            listed = await run(npmExecutable, ["list", "--global", "--depth=0", "--json", "taskbelay-codex"], { environment, timeout: 15_000 });
          } catch (error) {
            ...
            if (!report || Array.isArray(report) || report.errors?.length || report.dependencies?.["taskbelay-codex"]) throw error;
            listed = { stdout: error.stdout };
          }
          metadata = parseJSON(listed.stdout, "npm installed packages").dependencies?.["taskbelay-codex"] ?? null;
          if (metadata) stableVersion(metadata.version, "installed Codex version");
        } catch (error) {
          issues.push(issue("package_check_failed", error, "npm list --global --depth=0 taskbelay-codex"));
        }
      }
      let orphan = null;
      if (!status.available && !metadata) {
        try { orphan = await inspectOrphanRegistration(paths); }
        catch (error) { issues.push(issue("registration_check_failed", error, "taskbelay doctor --host codex")); }
      }
      ...
      if (orphan) issues.push({ code: "orphaned_registration", message: "Codex registration remains after its Adapter package was removed.", command: "taskbelay uninstall --host codex --yes" });
      if (!installed && !issues.length) issues.push({ code: "adapter_missing", message: "TaskBelay Codex Adapter is not installed.", command: "taskbelay install --host codex --yes" });
      if (installed && state !== "ready" && !issues.length) issues.push({ code: "registration_incomplete", message: "Codex Adapter registration is incomplete.", command: "taskbelay repair --host codex --yes" });
      return Object.freeze({
    ...
    async resolveTargetVersion(target) {
      ...
      const result = await run(npmExecutable, ["view", `taskbelay-codex@${target}`, "version", "--json"], { environment });
    ...
    async execute(operation, { targetVersion, observed, onProgress = () => {}, onStepStart = () => {} }) {
      ...
      if (operation === "uninstall") {
        ...
        try {
          ...
          if (!observed.orphanedRegistration) {
            ...
            await run(npmExecutable, ["uninstall", "--global", "taskbelay-codex"], { environment });
            completedSteps.push("codex.uninstall_package");
            onProgress("codex.uninstall_package");
          }
          return { changed: true, completedSteps };
        } catch (error) {
          throw partialError(error, completedSteps, observed.orphanedRegistration ? "taskbelay uninstall --host codex --yes" : "taskbelay repair --host codex --yes");
        }
      }
      ...
      try {
        ...
        const packageSource = localPackage?.path ?? `taskbelay-codex@${targetVersion}`;
        ...
        throw partialError(error, completedSteps, `taskbelay repair --host codex --version ${targetVersion} --yes`);
      }
    },
  });
}
