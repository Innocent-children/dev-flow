  ...
  return Object.freeze({
    ...
    async maintenanceTargets({ profile, observed }) {
      ...
      return {
        ...
        installedRuntime: await installedCoreRuntime(packageRoot, paths,
          { host: "deepseek", profile, packageName: "taskbelay-deepseek" }),
      };
    },
    ...
    async observe(profile) {
      ...
      if (receipt && contribution) {
        ...
        catch (error) { issues.push({ code: "core_check_failed", message: error.message, command: `taskbelay repair --host deepseek --profile ${profile} --yes` }); }
      }
      if (contribution && !receipt) issues.push({ code: "unmanaged_profile", message: "This Profile has an Adapter without a lifecycle receipt. Adopt it explicitly.", command: `taskbelay repair --host deepseek --profile ${profile} --adopt --yes` });
      if (receipt && !contribution && host.available) issues.push({ code: "registration_incomplete", message: "DeepSeek Adapter contribution is missing.", command: `taskbelay repair --host deepseek --profile ${profile} --yes` });
      if (!receipt && !contribution && host.available && !issues.length) issues.push({ code: "adapter_missing", message: "TaskBelay DeepSeek Adapter is not installed.", command: `taskbelay install --host deepseek --profile ${profile} --yes` });
    ...
    async resolveTargetVersion(target) {
      ...
      const result = await run(npmExecutable, ["view", `taskbelay-deepseek@${target}`, "version", "--json"], { environment });
    ...
    async execute(operation, { profile, targetVersion, observed, adopt = false, onProgress = () => {}, onStepStart = () => {} }) {
      ...
      if (operation === "uninstall") {
        ...
        try {
          ...
          await run(dshExecutable, ["plugin", "--profile", profile, "remove", "taskbelay-deepseek"], { environment });
          ...
          throw partialError(error, completedSteps, `taskbelay repair --host deepseek --profile ${profile} --yes`);
        }
      }
      ...
      if (observed.contribution && !observed.receipt && !adopt) throw nextStepError(
        "DeepSeek Profile requires explicit adoption", `taskbelay repair --host deepseek --profile ${profile} --adopt --yes`);
      const temporaryRoot = localPackage ? null : await mkdtemp(join(tmpdir(), "taskbelay-deepseek-"));
      const completedSteps = [];
      try {
        ...
        if (localPackage) {
          ...
          const packed = await run(npmExecutable, ["pack", `taskbelay-deepseek@${targetVersion}`, "--json"], { environment, cwd: temporaryRoot });
          ...
          if (report?.name !== "taskbelay-deepseek" || report?.version !== targetVersion || typeof report?.filename !== "string" || basename(report.filename) !== report.filename) {
        ...
        await writeProfileReceipt(paths, {
          profile,
          package_name: "taskbelay-deepseek",
        ...
        throw partialError(error, completedSteps, `taskbelay repair --host deepseek --profile ${profile} --version ${targetVersion} --yes`);
      } finally {
...
function profilePackageRoot(paths, profile, environment) {
  return join(resolve(environment.DSH_HOME || join(paths.homeDirectory, ".dsh")), "profiles", profile, "node_modules", "taskbelay-deepseek");
}

function runtimeCandidate(paths, profile, packageVersion, environment) {
  const packageRoot = profilePackageRoot(paths, profile, environment);
  return { source: `deepseek/${profile}`, packageName: "taskbelay-deepseek", packageVersion,
