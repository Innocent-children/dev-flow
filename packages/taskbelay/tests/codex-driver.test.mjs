...
test("local replacement removes an owned registration before changing package bytes", async () => {
  ...
  assert.deepEqual(calls, [["taskbelay-codex", "remove"], ["npm", "install"], ["taskbelay-codex", "setup"], ["taskbelay-codex", "status"]]);
...
test("Codex install and uninstall use exact package then lifecycle order", async () => {
  ...
  assert.deepEqual(calls.slice(0, 3), [
    ["npm", ["install", "--global", "taskbelay-codex@0.8.0"]],
    ["taskbelay-codex", ["setup", "--json"]],
    ["taskbelay-codex", ["status", "--json"]],
  ]);
  ...
  assert.deepEqual(calls, [
    ["taskbelay-codex", ["remove", "--json"]],
    ["npm", ["uninstall", "--global", "taskbelay-codex"]],
  ]);
});

test("Codex local package bypasses registry lookup and installs the exact tarball", async () => {
  const calls = [];
  const artifact = "/tmp/taskbelay-codex-local.tgz";
  const driver = createCodexDriver({
...
test("Codex uninstall keeps the package when remove fails", async () => {
  ...
  const driver = createCodexDriver({ run: async (executable, arguments_) => {
    calls.push([executable, arguments_]);
    if (executable === "taskbelay-codex") throw new Error("WebUI did not stop");
    return { stdout: "{}\n", stderr: "" };
  } });
  ...
  await assert.rejects(() => driver.execute("uninstall", {
    ...
    assert.equal(error.nextStep, "taskbelay repair --host codex --yes");
    return true;
  });
...
test("broken Core self-check preserves npm version and remains repairable", async () => {
  ...
  const driver = createCodexDriver({ run: async (command, args) => {
    ...
    if (args[0] === 'list') return { stdout: JSON.stringify({ dependencies: { 'taskbelay-codex': { version: '1.2.3' } } }) };
    if (args[0] === 'status') {
  ...
  assert.equal(calls.some(call => call.includes('taskbelay-codex@1.2.3')), true);
});
