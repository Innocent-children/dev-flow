...
async function fixture(t) {
  const root = await realpath(await mkdtemp(join(tmpdir(), "taskbelay-orphan-")));
  ...
  const paths = { productRoot: join(root, "product"), defaultDataDirectory: join(root, "product", "data"),
    ...
    runtimeDirectory: platform === "win32" ? "win32-x64" : "darwin-arm64", runtimeExecutable: platform === "win32" ? "taskbelay.exe" : "taskbelay" };
  const packageRoot = join(root, "npm", "taskbelay-codex");
  const receiptPath = join(paths.productRoot, "registrations", "codex.json");
  const registration = { marketplace_name: "taskbelay-local", marketplace_root: packageRoot,
    plugin_name: "taskbelay-codex", plugin_selector: "taskbelay-codex@taskbelay-local", plugin_root: join(packageRoot, "plugin") };
  const receipt = { product: { name: "taskbelay-codex", version: "0.9.3" }, registration,
  ...
  await writeFile(join(paths.defaultDataDirectory, "taskbelay.db"), "preserve task data");
  ...
  const run = async (command, args) => {
    ...
    if (command === "taskbelay-codex") throw Object.assign(new Error("not installed"), { code: "ENOENT" });
...
test("missing package remains discoverable and its registration can be uninstalled repeatedly", async t => {
  ...
  assert.equal(await readFile(join(f.paths.defaultDataDirectory, "taskbelay.db"), "utf8"), "preserve task data");
...
test("orphan cleanup reports partial removal and resumes without the removed plugin", async t => {
  ...
  await assert.rejects(driver.execute("uninstall", { observed: await driver.observe() }), error => {
    ...
    assert.equal(error.nextStep, "taskbelay uninstall --host codex --yes");
