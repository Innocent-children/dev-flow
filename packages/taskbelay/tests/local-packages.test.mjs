...
test("a declared development distribution binds both artifact paths and bytes", async t => {
  const root = await mkdtemp(join(tmpdir(), "taskbelay-local-source-"));
  ...
  await writeFile(join(root,"package.json"),JSON.stringify({taskBelayLocalPackages:entries}));
  ...
  await assert.rejects(runLifecycle(parseArguments(["install","--host","all","--yes"]),{
    ...
    claudeDriver:{observe(){hostCalls++;}},
    zcodeDriver:{observe(){hostCalls++;}},
  }),/digest mismatch/);
