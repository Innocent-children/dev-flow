...
test("unified CLI discovers a Windows x64 Codex Core and uses LOCALAPPDATA", {
  ...
  const root = await realpath(await mkdtemp(join(tmpdir(), "taskbelay-manager-windows-")));
  const home = join(root, "home");
  const localAppData = join(home, "AppData", "Local");
  const packageRoot = join(root, "node_modules", "taskbelay-codex");
  const runtimePath = join(packageRoot, "runtime", "win32-x64", "taskbelay.exe");
  const dataDirectory = join(localAppData, "taskbelay", "data");
  const receiptPath = join(localAppData, "taskbelay", "registrations", "codex.json");
  await Promise.all([
    mkdir(join(packageRoot, "runtime", "win32-x64"), { recursive: true }),
    mkdir(dataDirectory, { recursive: true }),
    mkdir(join(localAppData, "taskbelay", "registrations"), { recursive: true }),
  ]);
  await copyFile(windowsCore, runtimePath);
  const version = (await readFile(new URL("../../../CORE_VERSION", import.meta.url), "utf8")).trim();
  await writeFile(join(packageRoot, "package.json"), `${JSON.stringify({ name: "taskbelay-codex", version: "0.8.5" })}\n`);
  await writeFile(receiptPath, `${JSON.stringify({
    product: { name: "taskbelay-codex", version: "0.8.5", core_version: version, codex_compatibility: ">=0.147.0" },
  ...
  delete environment.TASKBELAY_DATA_DIR;
  const paths = await resolveManagerPaths({ homeDirectory: home, environment, platform: "win32", arch: "x64" });
  assert.equal(paths.productRoot, join(localAppData, "taskbelay"));
  assert.equal(paths.managerRoot, join(localAppData, "taskbelay"));
  assert.equal(paths.trashDirectory, join(localAppData, "taskbelay", "trash"));
...
test("Windows recoverable cleanup moves exact data into the manager quarantine", {
  ...
  const root = await realpath(await mkdtemp(join(tmpdir(), "taskbelay-manager-windows-trash-")));
  const home = join(root, "home");
  const localAppData = join(home, "AppData", "Local");
...
test("unified CLI discovers a native Claude-only Core installation", {
  ...
  const root = await realpath(await mkdtemp(join(tmpdir(), "taskbelay-claude-runtime-")));
  t.after(() => rm(root, { recursive: true, force: true }));
  ...
  await copyFile(windowsCore, runtimePath);
  await writeFile(join(packageRoot, "package.json"), JSON.stringify({ name: "taskbelay-claude", version: "0.1.0" }));
  const environment = { ...process.env, USERPROFILE: home, HOME: home, LOCALAPPDATA: localAppData, DSH_HOME: join(root, "dsh"), TASKBELAY_DATA_DIR: "" };
  const paths = await resolveManagerPaths({ homeDirectory: home, environment, platform: "win32", arch: "x64" });
  ...
  await writeFile(receiptPath, JSON.stringify({ product: { name: "taskbelay-claude", version: "0.1.0", core_version: coreVersion }, host: { surface: "claude-cli", version: "2.1.270", os: "win32", arch: "x64" }, paths: { package_root: packageRoot, runtime_path: runtimePath, data_dir: paths.defaultDataDirectory, receipt_path: receiptPath, config_root: join(home, ".claude") } }));
