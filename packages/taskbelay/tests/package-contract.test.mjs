...
import { runTaskBelay } from "../lib/runtime.mjs";
...
test("manifest exposes one dependency-free public macOS arm64 and Windows x64 TaskBelay package", async () => {
  ...
  assert.deepEqual(manifest.bin, { "taskbelay": "bin/taskbelay.mjs" });
...
test("the public command inventory names the desktop pet surface", async () => {
  ...
  assert.equal((await runTaskBelay(["help"], { stdout })).code, 0);
  assert.match(stdout.text, /^ {2}taskbelay pet start\|stop$/mu);
  ...
  const invalid = spawnSync(process.execPath, [join(packageRoot, "bin", "taskbelay.mjs"), "pet", "status"], {
    encoding: "utf8",
    env: { ...process.env, LC_ALL: "en_US.UTF-8" },
  });
  assert.equal(invalid.status, 2);
  assert.equal(invalid.stdout, "");
  assert.equal(invalid.stderr, "taskbelay: invalid arguments; the supported forms are taskbelay pet start and taskbelay pet stop\n");
});
