...
test("manifest-listed lifecycle modules run without repository files", async t => {
  ...
  const root = await realpath(await mkdtemp(join(tmpdir(), "taskbelay-staged-cli-")));
  ...
  const result = await execFile(process.execPath, [join(root, "bin/taskbelay.mjs"), "help"], { cwd: root, encoding: "utf8" });
