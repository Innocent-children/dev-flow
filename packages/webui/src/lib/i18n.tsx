...
const en = {
  ...
  "shell.overview": "Overview", "shell.tasks": "Tasks", "shell.openTask": "Start Task", "shell.system": "System",
  ...
  "scope.changed": "Task changed paths", "scope.decisions": "Developer decisions", "scope.coveredTools": "Write-before tools",
  "scope.needsDecision": "These paths are not covered by the current plan or a consumed authorization.",
  "scope.boundary": "The Host checks listed structured tools before writing. Core checks all Task-introduced paths before TEST and DONE; shell, external-process, and specialized-tool writes may only be found by that final check.",
  ...
  "runtime.resetBody": "Pre-Feature Task data is unchanged. Review the exact CLI reset plan before permanent cleanup.",
  ...
  "runtime.unavailableBody": "No reusable local WebUI process is responding for this data root.", "runtime.resetTitle": "Reset stays in the CLI.",
  "runtime.resetGuidance": "Inspect the target-bound plan first. The browser never performs this deletion.",
  "readiness.ready": "Ready", "readiness.read_only": "Read-only", "readiness.reset_required": "Reset required",
...
const zh: Messages = {
  ...
  "scope.eyebrow":"文件范围","scope.ready":"修改路径已有说明","scope.blocked":"修改路径需要处理","scope.explained":"可以进行最终范围检查","scope.unexplained":"{count} 个未说明路径","scope.expected":"计划路径","scope.changed":"任务实际修改路径","scope.decisions":"用户决定","scope.coveredTools":"写入前检查工具","scope.needsDecision":"以下路径不在当前计划内，也没有已使用的明确授权。","scope.boundary":"Host 会在列出的结构化工具写入前检查；Core 会在进入测试和完成任务前核对全部任务修改路径。Shell、外部进程和专用工具的写入可能只能在最终检查时发现。",
  "lifecycle.eyebrow":"任务管理","lifecycle.title":"管理任务","lifecycle.cancel":"取消任务","lifecycle.restore":"取消归档","lifecycle.archive":"归档任务","lifecycle.purge":"永久删除","lifecycle.cancelEyebrow":"取消任务","lifecycle.cancelTitle":"确定取消这个任务？","lifecycle.cancelBody":"取消原因会被记录，任务状态将变为 CANCELLED，并释放占用的仓库。","lifecycle.reason":"原因","lifecycle.continue":"暂不取消","lifecycle.cancelling":"正在取消…","lifecycle.confirmCancel":"确认取消","lifecycle.failure":"任务管理操作失败","purge.failure":"删除失败","purge.eyebrow":"永久删除","purge.title":"确定永久删除这个任务？","purge.body":"任务、操作记录、仓库占用和归档状态都会被删除，且无法恢复。","purge.typeID":"请输入完整的任务 ID","purge.keep":"保留任务","purge.busy":"正在删除…","purge.confirm":"永久删除",
  ...
  "system.eyebrow":"运行状态","system.title":"系统状态","system.lede":"查看本机服务、数据目录和连接状态。","system.runtime":"本机服务","system.unavailable":"不可用","system.returnShell":"请在终端运行 dev-flow webui status 查看详情。","system.loading":"正在读取系统状态","runtime.dataRoot":"数据目录标识","runtime.url":"访问地址","runtime.notAvailable":"不可用","runtime.readyBody":"本机服务运行正常，可以查看和操作任务。","runtime.readOnlyBody":"任务可以查看，但数据目录恢复可写前无法进行操作。","runtime.resetBody":"旧任务数据没有被修改。永久清理前，请先在终端查看准确的 reset 计划。","runtime.incompatibleBody":"检测到另一个不兼容的本机服务，请使用启动它的程序停止服务。","runtime.unavailableBody":"当前数据目录没有正在运行的 WebUI 服务。","runtime.resetTitle":"请在终端执行 Reset。","runtime.resetGuidance":"先核对要清理的文件，浏览器不会执行删除操作。",
  "readiness.ready":"运行正常","readiness.read_only":"只能查看","readiness.reset_required":"需要清理旧数据","readiness.incompatible":"版本不兼容","readiness.unavailable":"服务不可用","state.active":"进行中","state.blocked":"暂时受阻","state.done":"已完成","state.cancelled":"已取消","api.session":"当前页面已失效，请刷新后重试。","api.requestFailed":"请求失败，状态码 {status}",
