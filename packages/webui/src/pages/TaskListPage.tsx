  ...
  return (
    <div className="page-stack">
      ...
      <form key={query.toString()} className="filter-panel" onSubmit={submit} aria-label={t("list.filtersAria")}>
        ...
        <label className="filter-host">{t("list.host")}<SelectField name="host" ariaLabel={t("list.host")} defaultValue={query.get("host") ?? ""} options={[{ value: "", label: t("list.allHosts") }, { value: "codex", label: "Codex" }, { value: "deepseek", label: "DeepSeek" }, { value: "claude", label: "Claude Code" }, { value: "zcode", label: "ZCode" }]} /></label>
      ...
      {data === null ? <Loading label={t("list.loading")} /> : <section className="surface task-table-surface">
        ...
        {data.items.length === 0 ? <Empty title={t("list.emptyTitle")} detail={t("list.emptyDetail")} /> : <div className="table-scroll"><table><caption className="sr-only">{t("list.resultsTitle")}</caption><thead><tr><th>{t("list.task")}</th><th>{t("repository.worktree")}</th><th>{t("list.state")}</th><th>{t("list.host")}</th><th>{t("list.revision")}</th><th>{t("list.updated")}</th></tr></thead><tbody>{data.items.map((task) => <tr key={task.task_id}><td data-label={t("list.task")}><AppLink href={`/tasks/${encodeURIComponent(task.task_id)}`}><strong>{task.request_summary}</strong><code>{task.task_id}</code></AppLink></td><td className="worktree-cell" data-label={t("repository.worktree")}><span>{t("repository.group")} <code>{task.repository_group_id.slice(0, 12)}</code></span><small title={task.worktree_path}>{task.worktree_path}</small></td><td data-label={t("list.state")}><div className="table-status"><span className={`status-badge ${task.lifecycle}`}><span className="status-symbol" aria-hidden="true" />{t(stateKey(task.lifecycle))}</span>{task.archived && <span className="archive-label">{t("list.archived")}</span>}</div><small className="stage-cell"><span>{nodeLabel(task.current_node, language)}</span><code>{task.current_node}</code></small></td><td data-label={t("list.host")}><span className="host-name">{task.origin_host}</span></td><td data-label={t("list.revision")}><code>r{task.revision}</code></td><td data-label={t("list.updated")}><time dateTime={task.updated_at}>{formatDate(task.updated_at, language)}</time></td></tr>)}</tbody></table></div>}
