...
export function OpenTaskPage() {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const result = await resumeTask({
        execution_host: String(data.get("execution_host")) as "codex" | "deepseek" | "claude" | "zcode",
  ...
  return <div className="page-stack narrow-page">
    ...
    <form className="surface lifecycle-form" onSubmit={submit}>
      <fieldset><legend>{t("open.scope")}</legend><div className="form-grid"><label>{t("open.executionHost")}<SelectField name="execution_host" ariaLabel={t("open.executionHost")} defaultValue="codex" options={[{ value: "codex", label: "Codex" }, { value: "deepseek", label: "DeepSeek" }, { value: "claude", label: "Claude Code" }, { value: "zcode", label: "ZCode" }]} /></label><label>{t("open.worktreePath")}<input name="repository_path" placeholder="/absolute/path/to/original/task-worktree" required /></label></div></fieldset>
