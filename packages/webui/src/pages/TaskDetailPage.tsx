...
export function TaskDetailPage({ taskID }: { taskID: string }) {
  ...
  return <div className="page-stack" data-task-revision={data.summary.revision} data-stale={stale || error !== ""}>
    ...
    {data.current_action !== null && data.current_action.action_kind === "RESOLVE_BLOCKER" && data.blocker !== null ? <BlockerPanel taskID={data.summary.task_id} revision={data.summary.revision} action={data.current_action} blocker={data.blocker} disabled={data.readiness !== "ready" || stale || error !== ""} onChanged={refreshAfterMutation} pendingActionID={data.pending_action_id} /> : data.current_action !== null && <ActionPanel pendingActionID={data.pending_action_id} taskID={data.summary.task_id} revision={data.summary.revision} action={data.current_action} disabled={data.readiness !== "ready" || stale || error !== ""} onChanged={refreshAfterMutation} />}
...
function KnownFailureNotice({ records }: { records: TaskDetailResponse["records"] }) {
  const { t } = useI18n();
  const test = records.find((record) => record.kind === "test");
  if (test === undefined) return null;
  let acceptance: { summary: string; failed_checks: string[] } | undefined;
  try { acceptance = JSON.parse(test.value).known_failure_acceptance; } catch { return null; }
  if (acceptance === undefined || acceptance === null) return null;
  return <section className="notice warning" role="status"><strong>{t("verification.acceptedFailures", { count: acceptance.failed_checks.length })}</strong><p>{acceptance.summary}</p><ul>{acceptance.failed_checks.map((name) => <li key={name}>{name}</li>)}</ul></section>;
}
