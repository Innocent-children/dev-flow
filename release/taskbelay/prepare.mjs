...
export async function verifyWindowsApplication(application, version) {
  const executable = await readFile(join(application, "TaskBelayPet.exe"));
...
export async function verifyPackage(directory, expectedManifest, expectedFiles) {
  ...
  if (JSON.stringify(manifest) !== JSON.stringify(expectedManifest) || manifest.taskBelayLocalPackages) throw new Error("desktop package manifest differs");
...
export async function prepareTaskBelay({ outputRoot, sourceCommit }) {
  ...
  try {
    ...
    const record = {
      release: { product: "taskbelay", version: manifest.version, source_commit: sourceCommit },
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  if (args.length !== 2 || args[0] !== "--output") throw new Error("Usage: node release/taskbelay/prepare.mjs --output <absolute-directory>");
  const sourceCommit = (await run("git", ["-C", root, "rev-parse", "HEAD"])).stdout.trim();
  prepareTaskBelay({ outputRoot: args[1], sourceCommit }).then(
