...
async function directory(t) {
  const value = await mkdtemp(join(tmpdir(), "taskbelay-release-contract-"));
...
test("Windows application validation refuses non-PE and non-x64 executables", async t => {
  const stage = await directory(t);
  await writeFile(join(stage, "TaskBelayPet.exe"), "not an application");
  ...
  await writeFile(join(stage, "TaskBelayPet.exe"), pe);
