...
export function releaseOutputNames(product, version, coreVersion) { return [`dev-flow-core-${coreVersion}-darwin-arm64`, `${names[product]}-${version}.tgz`, "release-manifest.json", "SHA256SUMS"].sort(); }
export async function prepareRelease({ product, repositoryRoot, sourceCommit, sourceTree, firstTarball, secondTarball, outputDirectory }) {
  ...
  const tarballName=`${names[product]}-${version}.tgz`,coreName=`dev-flow-core-${coreVersion}-darwin-arm64`;
  ...
  const artifacts=[];for(const [kind,name] of [["npm_tarball",tarballName],["core_binary",coreName]])artifacts.push({kind,relative_path:name,sha256:await sha(join(outputDirectory,name))});
  ...
  const checksumNames=[tarballName,coreName,"release-manifest.json"];
