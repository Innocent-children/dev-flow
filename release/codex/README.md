...
```bash
node scripts/build-host-release.mjs --product codex --output "<ABSOLUTE_DIRECTORY>"
```
...
Resume is bound to product `codex`, both product versions, Tag, source commit/tree and
artifact digests. No previous release Tag is required. Mismatched or old-format directories fail before remote
...
Before its Tag, npm and GitHub Release operations, the shared publisher runs the local artifact check
for both initial publication and resume. Version-file commits and pushes precede artifact preparation.
