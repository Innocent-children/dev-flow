...
- Bevor Core eine Action-Einreichung vormerkt, prüft es auch die Semantik des Knotenergebnisses gegen die aktuelle Task. Fehler bei revision, record, evidence-Mengen und acceptance, deren Werte exakt aus Core kopiert werden können, liefern `allowed_paths` für genau eine begrenzte Korrektur. Testergebnisse, Benutzerbestätigungen und Arbeitsinhalte erhalten nur Feldinformationen und keine automatische Korrekturberechtigung.
