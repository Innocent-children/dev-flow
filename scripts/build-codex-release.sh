...
DEV_FLOW_RELEASE_CHANNEL="$release_channel" node - "$repository_root" "$codex_version" "$core_version" <<'NODE'
...
if (!expected.test(codexVersion) || !stable.test(coreVersion)) throw new Error("Codex version must match the release channel and Core must be MAJOR.MINOR.PATCH");
