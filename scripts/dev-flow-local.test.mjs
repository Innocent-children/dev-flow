...
test("Host source-only command archives regenerate stale copies from shared sources", async t => {
  ...
  await writeFile(join(source, "skills/dev-flow/core/example.md"), 'Packaged Host: {{host}}\n');
  ...
  for (const product of ["codex", "claude", "zcode"]) {
    ...
    const skillDirectory = product === "zcode" ? "skills/dev-flow" : "plugin/skills/dev-flow";
    ...
    const manifest = { name: `dev-flow-${product}`, version: "0.0.0", files: ["LICENSE", reference, ...hostCommandFiles.map(path => `lib/${path}`)] };
