...
import { prepareTaskBelay } from "../release/taskbelay/prepare.mjs";
...
function parseArguments(arguments_) {
  ...
  if (!versionPattern.test(version ?? "") || !isAbsolute(output ?? "") || values.get("--confirm") !== `taskbelay-v${version}`) {
    throw new Error("exact taskbelay release selection required");
  }
...
async function alignVersion(targetVersion) {
  const manifestPath = join(root, "packages/taskbelay/package.json");
  const manifest = JSON.parse(await readFile(manifestPath, "utf8"));
  if (!versionPattern.test(manifest.version ?? "")) throw new Error("TaskBelay CLI package version is invalid");
  ...
  await run("git", ["add", "packages/taskbelay/package.json"]);
  await run("git", ["commit", "-m", `release(taskbelay): v${targetVersion}`]);
...
async function prepare(selection) {
  ...
  await prepareTaskBelay({ outputRoot: selection.output, sourceCommit });
  return sourceCommit;
}

async function main(selection) {
  await ensureCleanSynchronizedMain();
  await run(process.execPath, ["--test", "release/publish.test.mjs", "packages/taskbelay/tests/package-contract.test.mjs", "release/taskbelay/prepare.test.mjs"]);
  await alignVersion(selection.version);
  const sourceCommit = await prepare(selection);
  return publishRelease({ product: "taskbelay", version: selection.version, directory: selection.output, sourceCommit });
}

main(parseArguments(process.argv.slice(2))).then(
  (result) => process.stdout.write(`${JSON.stringify(result)}\n`),
  (error) => {
    process.stderr.write(`release-taskbelay: ${error.message}\n`);
    process.exitCode = 1;
  },
);
