...
export async function prepareRelease({
  ...
  const { deepseekVersion, coreVersion } = await readSourceVersions(root);
  validateReleaseMode(verificationMode, basedOnRelease, deepseekVersion);
  const expectedTarballName = `dev-flow-deepseek-${deepseekVersion}.tgz`;
  ...
  try {
    const first = await inspectPackageTarball(firstTarball, { repositoryRoot: root, version: deepseekVersion });
    const second = await inspectPackageTarball(secondTarball, { repositoryRoot: root, version: deepseekVersion });
    ...
    const manifest = {
      release: {
        ...
        tag: `deepseek-v${deepseekVersion}`,
    ...
    validateManifest(manifest, { deepseekVersion, coreVersion, sourceCommit, sourceTree });
    ...
    if (!arraysEqual(stagedNames, releaseOutputNames(deepseekVersion, coreVersion))) {
    ...
    return {
      ...
      build_count: 2,
      output_files: releaseOutputNames(deepseekVersion, coreVersion),
    };
  } finally {
...
export async function verifyReleaseDirectory({
  ...
  const deepseekVersion = manifest?.release?.version;
  const coreVersion = manifest?.release?.core_version;
  const expectedNames = releaseOutputNames(deepseekVersion, coreVersion);
  ...
  if (sourceVersions.deepseekVersion !== deepseekVersion || sourceVersions.coreVersion !== coreVersion) {
  ...
  const tarballName = `dev-flow-deepseek-${deepseekVersion}.tgz`;
  ...
  const inspected = await inspectPackageTarball(tarballPath, { repositoryRoot: root, version: deepseekVersion });
...
export async function inspectPackageTarball(tarballPath, { repositoryRoot, version }) {
  ...
  try {
    ...
    for (const relativePath of actualFiles) {
      ...
      const expectedMode = relativePath === "runtime/darwin-arm64/dev-flow" ? "0755" : "0644";
...
export function validateManifest(manifest, { deepseekVersion, coreVersion, sourceCommit, sourceTree }) {
  ...
  if (
    manifest.release.product !== "deepseek" ||
    manifest.release.version !== deepseekVersion ||
    manifest.release.core_version !== coreVersion ||
    manifest.release.tag !== `deepseek-v${deepseekVersion}` ||
  ...
  validateReleaseMode(manifest.release.verification_mode, manifest.release.based_on_release, deepseekVersion);
  ...
  for (const file of manifest.package_files) {
    ...
    const expectedMode = file.path === "runtime/darwin-arm64/dev-flow" ? "0755" : "0644";
  ...
  assertExactKeys(support, ["os", "arch", "actual_dsh_version", "compatible_dsh_range", "package_sha256", "core_sha256", "journey_result", "journey_observed_at", "verification_mode", "based_on_release", "notes"], "support entry");
  if (support.os !== "darwin" || support.arch !== "arm64" || support.compatible_dsh_range !== DEEPSEEK_RANGE || !["pending", "passed", "failed", "blocked"].includes(support.journey_result)) throw new Error("support entry differs from the darwin-arm64 contract");
  ...
  const artifactByKind = new Map(manifest.artifacts.map((artifact) => [artifact.kind, artifact]));
  if (artifactByKind.get("core_binary")?.name !== `dev-flow-core-${coreVersion}-darwin-arm64` || artifactByKind.get("npm_tarball")?.name !== `dev-flow-deepseek-${deepseekVersion}.tgz`) throw new Error("release artifact names differ from product versions");
...
export function buildSupportMatrixFromFinalJourney({ manifest, evidence }) {
  ...
  return [{
    os: "darwin",
    arch: "arm64",
    actual_dsh_version: validated.dsh_version,
    compatible_dsh_range: DEEPSEEK_RANGE,
    ...
    notes: "Native registry-package DeepSeek lifecycle smoke passed install, version/Core handshake, setup, removal, uninstall, and repository-unchanged gates.",
  }];
}
...
export function validatePublicationRecord(record, {
  ...
  if (record.release.product !== "deepseek" || record.release.version !== deepseekVersion || record.release.core_version !== coreVersion || record.release.tag !== `deepseek-v${deepseekVersion}` || record.release.source_commit !== sourceCommit || record.release.source_tree !== sourceTree || record.release.verification_mode !== verificationMode || record.release.based_on_release !== basedOnRelease) throw new Error("publication record release identity differs from the manifest/source");
  ...
  if (record.npm.name !== "dev-flow-deepseek" || record.npm.version !== deepseekVersion || typeof record.npm.published !== "boolean" || typeof record.npm.verified !== "boolean") throw new Error("publication npm identity/state is invalid");
  ...
  assertExactKeys(record.final_journey, ["status", "actual_dsh_version", "observed_at", "summary"], "final journey state");
  if (!["pending", "passed", "failed", "blocked"].includes(record.final_journey.status) || (record.final_journey.actual_dsh_version !== null && (typeof record.final_journey.actual_dsh_version !== "string" || record.final_journey.actual_dsh_version.length > 64)) || (record.final_journey.observed_at !== null && !Number.isFinite(Date.parse(record.final_journey.observed_at))) || typeof record.final_journey.summary !== "string" || record.final_journey.summary.length > 1000) throw new Error("publication final journey state is invalid or unbounded");
...
export function validatePreparedPublicationState(record) {
  ...
  if (record.final_journey.status !== "pending" || record.final_journey.actual_dsh_version !== null || record.final_journey.observed_at !== null) throw new Error("prepared publication record must keep final journey pending");
}
...
export async function rewriteChecksums(directory, deepseekVersion, coreVersion) {
  const names = [`dev-flow-core-${coreVersion}-darwin-arm64`, `dev-flow-deepseek-${deepseekVersion}.tgz`, "release-manifest.json"].sort();
  const lines = [];
...
export function scanTextContent(text, { repositoryRoot, record, field = "" } = {}) {
  ...
  for (const marker of ["example-token-marker", "authorization: bearer", "//registry.npmjs.org/:_authtoken", ".npmrc", "dev-flow-codex", "raw prompt marker", "raw command output marker"]) {
...
export function initialPublicationRecord({ deepseekVersion, coreVersion, verificationMode, basedOnRelease, sourceCommit, sourceTree, manifestSHA256, tarballSHA256, observedAt }) {
  ...
  return {
    release: { product: "deepseek", version: deepseekVersion, core_version: coreVersion, tag: `deepseek-v${deepseekVersion}`, source_commit: sourceCommit, source_tree: sourceTree, verification_mode: verificationMode, based_on_release: basedOnRelease },
    overall_status: "prepared",
    manifest_sha256: manifestSHA256,
    steps,
    npm: { name: "dev-flow-deepseek", version: deepseekVersion, published: false, integrity: null, tarball_sha256: null, verified: false },
    github: { tag: `deepseek-v${deepseekVersion}`, tag_target: null, release_id: null, draft: false, published: false, assets: [] },
    final_journey: { status: "pending", actual_dsh_version: null, observed_at: null, summary: "Final registry-package journey has not run." },
...
function validatePublicPackageManifest(manifest, version) {
  ...
  if (
    manifest.name !== "dev-flow-deepseek" ||
...
function parseVerifyArguments(arguments_) {
  ...
  if (normalized.length !== 2 || normalized[0] !== "--directory" || !normalized[1]) throw new Error("usage: verify-deepseek-release.mjs --directory ABSOLUTE_RELEASE_DIRECTORY");
  return normalized[1];
}
...
if (isMainModule()) {
  try {
    ...
    process.stderr.write(`verify-deepseek-release: ${boundedError(error)}\n`);
    process.exitCode = 1;
  }
}
