...
const skillDirectories = Object.freeze({ codex: "packages/codex/plugin/skills/dev-flow", deepseek: "packages/deepseek/skills/dev-flow", claude: "packages/claude/plugin/skills/dev-flow" });
...
export async function sharedSkillReferences({ root = repositoryRoot, host } = {}) {
  if (!Object.hasOwn(skillDirectories, host)) throw new Error("Skill host must be codex, deepseek or claude");
...
async function main(args) {
  ...
  throw new Error("Usage: node scripts/sync-skill-references.mjs [--check | --host codex|deepseek|claude --output SKILL_DIRECTORY]");
}
