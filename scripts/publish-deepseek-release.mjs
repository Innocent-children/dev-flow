...
import {
  ...
  writeJSONAtomic,
} from "./verify-deepseek-release.mjs";
import {
...
export async function runPublisher({
  ...
  const context = {
    ...
    tarballPath: join(releaseDirectory, `dev-flow-deepseek-${verified.version}.tgz`),
...
async function runProductionRegistryJourney(context, manifest, { runnerMode, evidenceName, validateEvidence }) {
  ...
  const dshExecutable = context.dshExecutable ?? await findExecutableInEnvironment("dsh", context.environment);
  ...
  try {
    ...
    const stdout = await runText(process.execPath, [runner,
      runnerMode,
      "--package", "dev-flow-deepseek",
      "--version", context.version,
      "--registry", OFFICIAL_REGISTRY,
      "--tarball-sha256", context.tarballSHA256,
      "--npm-integrity", context.npmIntegrity,
...
async function observeRemoteState(context, manifest, record) {
  ...
  const packageView = await runAllowFailure("npm", [
    "view", "dev-flow-deepseek", "name", "version", "maintainers", "dist-tags", "--json", `--registry=${OFFICIAL_REGISTRY}`,
  ], context);
  if (packageView.code === 0) {
    const owners = await runText("npm", ["owner", "ls", "dev-flow-deepseek", `--registry=${OFFICIAL_REGISTRY}`], context);
  ...
  if (release !== null) {
    const allowedAssets = new Set([
      ...
      `dev-flow-deepseek-${context.version}.tgz`,
  ...
  if (exactNPM !== null) {
    ...
    context.npmIntegrity = exactNPM.integrity;
...
async function ensurePublishedNPM(context, record, manifest) {
  ...
  if (observed === null && !record.npm.published) {
    ...
    completeStep(publishStep, {
      remoteID: `dev-flow-deepseek@${context.version}`,
  ...
  context.npmIntegrity = observed.integrity;
...
async function verifyRegistryTarball(context, observed, manifest) {
  ...
  try {
    const stdout = await runText("npm", [
      "pack", `dev-flow-deepseek@${context.version}`,
...
async function ensureFinalJourney(context, manifest, record) {
  ...
  record.final_journey = {
    status: "passed",
    actual_dsh_version: evidence.dsh_version,
...
function hasReusableFinalJourney(context, manifest, record) {
  ...
  if (
    ...
    || support?.actual_dsh_version !== record.final_journey.actual_dsh_version
...
async function prepareFinalManifest(context, manifest, record, evidence) {
  ...
  const generatedSupport = evidence.evidence_kind === FINAL_FIXTURE_EVIDENCE_KIND
    ? [{
      ...
      arch: "arm64",
      actual_dsh_version: evidence.dsh_version,
      compatible_dsh_range: evidence.compatible_dsh_range,
  ...
  manifest.support = existingSupport.journey_result === "passed"
    && existingSupport.actual_dsh_version === generatedSupport[0].actual_dsh_version
    ...
    && existingSupport.notes === generatedSupport[0].notes
      ? [existingSupport]
      : generatedSupport;
  record.final_journey.actual_dsh_version = manifest.support[0].actual_dsh_version;
  ...
  validateManifest(manifest, { deepseekVersion: context.version, coreVersion: context.coreVersion, sourceCommit: context.sourceCommit, sourceTree: context.sourceTree });
...
function assertFinalizationGates(context, manifest, record) {
  ...
  if (
    ...
    || support[0].actual_dsh_version !== record.final_journey.actual_dsh_version
...
async function observeExactNPM(context, { delayed }) {
  const result = await runAllowFailure("npm", [
    "view", `dev-flow-deepseek@${context.version}`, "version", "dist", "--json", `--registry=${OFFICIAL_REGISTRY}`,
  ], context);
  if (result.code !== 0) {
...
async function verifyNPMChannelTag(context) {
  const output = await runText("npm", [
    "view", "dev-flow-deepseek", "dist-tags", "--json", `--registry=${OFFICIAL_REGISTRY}`,
  ], context);
  let parsed;
  try {
...
async function checkpoint(path, record, manifest, context) {
  ...
  validatePublicationRecord(record, {
    deepseekVersion: context.version,
...
export function productionJourneyRunnerPath() {
  return join(repositoryRootFromModule(), "scripts", "run-deepseek-real-journey.mjs");
}
...
function parseArguments(arguments_) {
  ...
  if (directory === null) throw new Error("usage: publish-deepseek-release.mjs --directory ABSOLUTE_RELEASE_DIRECTORY [--source-root ABSOLUTE_FROZEN_SOURCE] [--confirm vVERSION]");
...
if (isMainModule()) {
  try {
    ...
    process.stderr.write(`publish-deepseek-release: ${boundedMessage(error)}\n`);
    process.exitCode = 1;
  }
}
