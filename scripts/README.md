...
| `pnpm run dev-flow:local` | 从当前源码打包四个 Adapter 和统一入口并进入与 `dev-flow` 相同的本地安装菜单 |
...
`dev-flow-codex`、`dev-flow-deepseek`、`dev-flow-claude` 与 `dev-flow-zcode` tarball，再从本地 tarball 启动统一安装菜单：
...
获取短期 npm 发布凭据，使用
ARM64 runner（Codex/DeepSeek 使用 `macos-15`，Dev Flow 桌面包使用带 Xcode 27 的 `xcode-27` 预览镜像）、Go `1.26.5`、Node.js `24.18.0` 和 pnpm `11.24.0`，按产品串行执行，并交叉构建、
...
Windows 桌面开发包同时携带完整的 Codex、DeepSeek、Claude 与 ZCode 安装包；构建复用 buildCoreRuntimes 和 stageAndPack，不生成缺少另一平台 Core 的临时特制 Adapter 包。安装统一入口后，`dev-flow install --host all --yes` 准备四个 Adapter 与桌面应用；ZCode 仍需按返回说明在 UI 中安装和启用。
