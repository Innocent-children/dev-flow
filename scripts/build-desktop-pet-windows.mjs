...
// Windows application assembly uses JavaScript and an explicit x64 Electron distribution.
export async function stageWindowsDesktopApplication({ application, version, dependenciesRoot, electronDirectory }) {
  const source = join(root, "packages", "desktop-pet", "windows");
  await cp(electronDirectory, application, {
    recursive: true,
  });
  await rename(
  ...
  for (const module of ["saxes", "xmlchars", "image-size"])
    await cp(
      join(dependenciesRoot, "node_modules", module),
  ...
  return await verifyDefaultArtwork(
...
export async function buildWindowsDesktopPet({ outputRoot }) {
  ...
  try {
    ...
    await writeFile(
      ...
      JSON.stringify(
        { ...manifest, devFlowLocalPackages, files: [...manifest.files, "local-packages/codex.tgz", "local-packages/deepseek.tgz"] },
        null,
        2,
      ) + "\n",
    );
    const application = join(packageRoot, applicationPath);
    const assets = await stageWindowsDesktopApplication({ application, version: manifest.version,
      dependenciesRoot: source, electronDirectory: join(source, "node_modules", "electron", "dist") });
