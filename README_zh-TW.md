...
首次回覆會評估請求，並詢問直接開發還是使用 Dev Flow。選擇 Dev Flow 後，確認本機或遠端
來源、起始分支、新任務分支，以及是否攜帶既有的本機修改。

任務隨後在專屬 Git 工作樹中進行，也就是為該任務準備的獨立目錄。Codex 在宿主支援時開啟
該工作樹；DeepSeek 會提供從新目錄重新啟動的命令。

### 3. 恢復並查看進度

會話重新啟動後，回到任務的原工作樹，明確要求繼續該任務。Dev Flow 會從已儲存的進度繼續。
原工作樹遺失或被替換時，任務會暫停，直到你恢復它或明確放棄任務。
...
桌面寵物顯示所選任務的已儲存狀態，並可開啟對應 WebUI。你可以選擇任務、自訂外觀、控制
動畫、調整大小，以及獨立啟動或停止寵物。使用前先完成上面的安裝與 Codex 或 DeepSeek 設定。
