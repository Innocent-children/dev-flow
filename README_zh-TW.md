...
macOS arm64 環境提供桌面寵物。安裝任一適配器（Codex 或 DeepSeek）後由外掛提供預置二進位檔案並自動就位至 `~/.dev-flow/pet/`，無需 Xcode 或 Swift 編譯器。設定好至少一個 Adapter 後，可顯示一個所選任務已儲存的階段與阻塞原因，點擊進入對應 WebUI。選單提供任務選擇、動畫開關和隱藏/顯示。階段代表 Core 儲存的狀態，不代表 Host 此刻正在執行，也不顯示完成百分比。結束程式會保留任務和 WebUI。更多說明見[命令參考](docs/COMMANDS.md#桌面宠物macos-arm64)，公開支援範圍仍以支援矩陣為準。

可從寵物選單匯入單張 PNG、Dev Flow 動畫套件或 Codex 精靈圖格式 1/2 的形象套件；選擇和匯入素材會在升級後保留。製作方式見[形象套件說明](docs/DESKTOP-PETS.md)。
