...
**累積任務經驗：** 每次進入理解確認時，任務 AI 自動回顧已有工作，儲存少量有價值的經驗後說明原因、適用條件與下次檢查方式。沒有值得記錄的內容可以略過，再次進入時更新已有經驗；仍需使用者明確確認理解。任務完成後匯出工作樹之外的 Markdown；WebUI 支援日後閱讀、依專案與關鍵字查找、使用者補充及重新匯出。詳見[任務經驗](docs/EXPERIENCES.md)。
