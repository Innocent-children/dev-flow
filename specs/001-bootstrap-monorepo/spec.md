...
3. **Given** either package dry-run with script execution disabled, **When** its contents are
   inspected, **Then** it contains only `package.json`, `README.md`, and pnpm's automatically
   included root Apache `LICENSE`, and the product manifest contains no non-empty `scripts`,
   executable entry, or runtime dependency.
...
1. **Given** a valid checkout, **When** baseline validation runs, **Then** all declared Go, pnpm,
   package, layout, and documentation checks pass without executing product-package or dependency
   lifecycle scripts during workspace installation or package dry-runs.
...
- A product manifest uses an npm lifecycle name such as `prepack` or `postpack`, or an arbitrary
  custom script name, in an attempt to bypass a lifecycle-name blocklist.
...
- **FR-016**: Host package manifests and README files MUST NOT claim a working runtime or host
  integration.
- **FR-017**: The repository MUST provide one local validation entry point covering `gofmt`,
  `go list ./...`, `go vet ./...`, `go test ./...`, workspace integrity, repository-layout
  contracts, package manifest/dry-pack checks, and Markdown links. Workspace installation and both
  product-package dry-packs MUST disable lifecycle script execution.
...
- **FR-019**: Repository-layout validation MUST reject nested Spec Kit roots, nested Go modules,
  undeclared executable roots, any non-empty product-package `scripts` field, product `bin` entries,
  product `publishConfig`, and product runtime dependencies during this feature. Violations MUST
  name the manifest path and the `scripts` field rather than maintaining a finite lifecycle-script
  blocklist.
...
- **SC-004**: The documented workspace install and package dry-run succeed with script execution
  disabled; neither dependency-package nor product-package lifecycle scripts execute, and no
  publication occurs.
