...
**积累任务经验：** 每次进入理解确认时，任务 AI 自动回顾已有工作，保存少量有价值的经验后讲解原因、适用条件和下次检查方法。没有值得记录的内容可以跳过，再次进入时更新已有经验；仍需用户明确确认理解。任务完成后导出工作树之外的 Markdown；WebUI 支持后续阅读、项目与关键词查找、用户补充及导出重试。详见[任务经验](docs/EXPERIENCES.md)。
