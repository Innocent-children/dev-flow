...
func ActionPayloadSchemas() []ActionPayloadSchema {
	...
	test := standardPayloadSchema(schemaObject([]string{"problem_class", "checks", "failed_items", "unverified_items", "manual_handoff_items", "findings", "budget_adjustment"}, map[string]any{
		"problem_class": schemaEnum("none", "implementation_failure", "design_failure", "requirement_gap"), "checks": map[string]any{"type": "array", "maxItems": 32, "items": check}, "failed_items": schemaList(), "unverified_items": schemaList(), "manual_handoff_items": schemaList(), "findings": schemaList(), "budget_adjustment": nullableSchema(budgetAdjustment), "known_failure_acceptance": nullableSchema(knownFailureAcceptanceSchema()),
	}))
