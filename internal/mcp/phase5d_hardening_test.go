...
func TestOptionalInputFieldsAcceptOmittedNullAndClosedNonNull(t *testing.T) {
	...
	valid := []struct {
		...
		{"open nonnull", ToolOpenTask, `{"host":"codex","repository_path":"/repo","workspace_origin":` + origin + `,"new_task":{"request":"Build feature","initial_scope":[],"initial_out_of_scope":[],"known_acceptance_criteria":[],"method_profile":"plain"}}`},
		{"open multi repository", ToolOpenTask, `{"host":"codex","repository_path":"/core","workspace_origin":` + origin + `,"primary_repository_key":"core","additional_repositories":[{"key":"docs","repository_path":"/docs","workspace_origin":` + docsOrigin + `}],"new_task":{"request":"Build feature","initial_scope":[],"initial_out_of_scope":[],"known_acceptance_criteria":[],"method_profile":"plain"}}`},
	...
	invalid := []struct {
		...
		{"open additional unknown", ToolOpenTask, `{"host":"codex","repository_path":"/core","workspace_origin":` + origin + `,"additional_repositories":[{"key":"docs","repository_path":"/docs","workspace_origin":` + docsOrigin + `,"unknown":true}],"new_task":{"request":"Build feature","initial_scope":[],"initial_out_of_scope":[],"known_acceptance_criteria":[],"method_profile":"plain"}}`},
		{"open null primary key", ToolOpenTask, `{"host":"codex","repository_path":"/core","workspace_origin":` + origin + `,"primary_repository_key":null,"new_task":{"request":"Build feature","initial_scope":[],"initial_out_of_scope":[],"known_acceptance_criteria":[],"method_profile":"plain"}}`},
		{"open null additions", ToolOpenTask, `{"host":"codex","repository_path":"/core","workspace_origin":` + origin + `,"additional_repositories":null,"new_task":{"request":"Build feature","initial_scope":[],"initial_out_of_scope":[],"known_acceptance_criteria":[],"method_profile":"plain"}}`},
...
func TestServerInfoProjectsHostPreferenceSnapshot(t *testing.T) {
	server := &Server{
		...
		hostPreferences: userconfig.Preferences{
			...
			ZCode:    userconfig.HostPreferences{CodebaseMemory: true},
		},
	}
	encoded := server.dispatch(context.Background(), ToolServerInfo, "server-info-preferences", []byte(`{}`))
	if encoded.IsError || !bytes.Contains(encoded.JSON, []byte(`"host_preferences":{"codex":{"codebase_memory":true},"deepseek":{"codebase_memory":false},"claude":{"codebase_memory":false},"zcode":{"codebase_memory":true}}`)) {
