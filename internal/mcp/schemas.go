...
func buildCatalog() []ToolDefinition {
	...
	workspaceOrigin := obj([]string{"mode", "source_type", "carry_changes", "remote_name", "base_branch", "base_commit", "task_branch", "provisioning_receipt_id"}, map[string]any{"mode": map[string]any{"const": "dedicated_worktree"}, "source_type": map[string]any{"enum": []string{"local", "remote"}}, "carry_changes": map[string]any{"type": "boolean"}, "remote_name": map[string]any{"type": "string", "maxLength": 128}, "base_branch": str(), "base_commit": map[string]any{"type": "string", "pattern": "^(?:[0-9a-f]{40}|[0-9a-f]{64})$"}, "task_branch": str(), "provisioning_receipt_id": id()})
