...
func graphPayloads() ([]any, []any) {
	...
	requirements := payloadSchema(obj([]string{"baseline", "unresolved_questions"}, map[string]any{"baseline": baselineReq, "unresolved_questions": list()}))
	...
	design := payloadSchema(obj([]string{"baseline", "findings"}, map[string]any{"baseline": nullable(designBase), "findings": list()}))
	...
	tasks := payloadSchema(obj([]string{"baseline", "findings"}, map[string]any{"baseline": nullable(tasksBase), "findings": list()}))
	implementation := payloadSchema(obj([]string{"task_plan_revision", "completed_work_item_ids", "changed_paths", "no_file_changes", "deviations", "findings"}, map[string]any{"task_plan_revision": map[string]any{"type": "integer", "minimum": 1}, "completed_work_item_ids": map[string]any{"type": "array", "items": id()}, "changed_paths": list(), "no_file_changes": map[string]any{"type": "boolean"}, "deviations": list(), "findings": list()}))
	...
	test := payloadSchema(obj([]string{"checks", "failed_items", "unverified_items", "manual_handoff_items", "findings"}, map[string]any{"checks": map[string]any{"type": "array", "maxItems": 32, "items": check}, "failed_items": list(), "unverified_items": list(), "manual_handoff_items": list(), "findings": list()}))
	...
	comprehension := payloadSchema(obj([]string{"explained_components", "unresolved_questions", "unnecessary_abstractions", "maintenance_risks", "user_confirmation", "findings"}, map[string]any{"explained_components": list(), "unresolved_questions": list(), "unnecessary_abstractions": list(), "maintenance_risks": list(), "user_confirmation": nullable(confirmation), "findings": list()}))
	refactor := payloadSchema(obj([]string{"changed_paths", "no_file_changes", "simplifications", "behavior_change_intended", "findings"}, map[string]any{"changed_paths": list(), "no_file_changes": map[string]any{"type": "boolean"}, "simplifications": list(), "behavior_change_intended": map[string]any{"type": "boolean"}, "findings": list()}))
	delivery := payloadSchema(obj([]string{"acceptance", "automated_evidence_ids", "manual_evidence_ids", "test_record_id", "comprehension_record_id", "unverified_items", "risks", "findings"}, map[string]any{"acceptance": map[string]any{"type": "array", "items": obj([]string{"criterion", "status"}, map[string]any{"criterion": str(), "status": map[string]any{"const": "satisfied"}})}, "automated_evidence_ids": map[string]any{"type": "array", "items": id()}, "manual_evidence_ids": map[string]any{"type": "array", "items": id()}, "test_record_id": id(), "comprehension_record_id": id(), "unverified_items": list(), "risks": list(), "findings": list()}))
	payloads := []any{requirements, design, tasks, implementation, test, comprehension, refactor, delivery}
	kinds := []string{"COMPLETE_REQUIREMENTS", "COMPLETE_DESIGN", "COMPLETE_TASKS", "COMPLETE_IMPLEMENTATION", "COMPLETE_TEST", "COMPLETE_COMPREHENSION_REVIEW", "COMPLETE_REFACTOR", "COMPLETE_DELIVERY"}
	branches := make([]any, len(kinds))
	for i, kind := range kinds {
		branches[i] = map[string]any{"title": kind, "properties": map[string]any{"action_kind": map[string]any{"const": kind}, "payload": payloads[i]}}
	}
...
func buildCatalog() []ToolDefinition {
	...
	probe := obj([]string{"operation_id", "process_id", "process_version", "process_definition_digest", "source_cursor", "expected_revision", "action_id", "action_kind", "repository_binding_digest", "payload"}, map[string]any{"operation_id": id(), "process_id": map[string]any{"const": "standard-development"}, "process_version": map[string]any{"const": 1}, "process_definition_digest": digest(), "source_cursor": id(), "expected_revision": map[string]any{"type": "integer", "minimum": 1}, "action_id": id(), "action_kind": id(), "repository_binding_digest": digest(), "payload": map[string]any{"anyOf": []any{payload, map[string]any{"type": "null"}}}})
	read := obj([]string{"host", "task_id", "operation_probe"}, map[string]any{"host": map[string]any{"enum": []string{"codex", "deepseek"}}, "task_id": id(), "operation_probe": map[string]any{"anyOf": []any{probe, map[string]any{"type": "null"}}}})
	applyProps := map[string]any{"request_id": id(), "host": map[string]any{"enum": []string{"codex", "deepseek"}}, "task_id": id(), "revision": map[string]any{"type": "integer", "minimum": 1}, "action_id": id(), "action_kind": id(), "process_id": map[string]any{"const": "standard-development"}, "process_version": map[string]any{"const": 1}, "process_definition_digest": digest(), "source_cursor": id(), "repository_binding_digest": digest(), "payload": payload, "recovery_apply": map[string]any{"anyOf": []any{obj([]string{"operation_id", "source_cursor"}, map[string]any{"operation_id": id(), "source_cursor": id()}), map[string]any{"type": "null"}}}}
	apply := obj([]string{"request_id", "host", "task_id", "revision", "action_id", "action_kind", "process_id", "process_version", "process_definition_digest", "source_cursor", "repository_binding_digest", "payload", "recovery_apply"}, applyProps)
	apply["allOf"] = []any{map[string]any{"oneOf": branches}}
	open := obj([]string{"host", "repository_path", "new_task"}, map[string]any{"host": map[string]any{"enum": []string{"codex", "deepseek"}}, "repository_path": str(), "new_task": map[string]any{"anyOf": []any{newTask, map[string]any{"type": "null"}}}})
