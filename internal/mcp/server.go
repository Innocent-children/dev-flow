...
func NewServer(service *application.Service, version string, options *ServerOptions) (*Server, error) {
	...
	for _, d := range catalog {
		d := d
		s.sdk.AddTool(&sdk.Tool{Name: d.Name, Description: d.Description, InputSchema: json.RawMessage(d.InputSchema)}, func(ctx context.Context, r *sdk.CallToolRequest) (*sdk.CallToolResult, error) {
			raw := json.RawMessage(`{}`)
...
func (s *Server) dispatch(ctx context.Context, tool string, id domain.ID, raw []byte) EncodedResult {
	...
	case ToolOpenTask:
		...
		return EncodeSuccess(string(id), tool, map[string]any{"created": r.Created, "task": projectTask(r.Task)})
	case ToolGetTask:
		var w readWire
		_ = decodeClosed(raw, &w)
		r, err := s.application.GetTask(ctx, application.GetTaskRequest{Host: w.Host, TaskID: w.TaskID})
		if err != nil {
			return EncodeError(string(id), tool, err)
		}
	...
	case ToolGetNextAction:
		var w readWire
		_ = decodeClosed(raw, &w)
		r, err := s.application.GetNextAction(ctx, application.GetNextActionRequest{Host: w.Host, TaskID: w.TaskID})
		if err != nil {
			return EncodeError(string(id), tool, err)
		}
		return EncodeSuccess(string(id), tool, r)
	case ToolApplyAction:
		var w applyWire
		_ = decodeClosed(raw, &w)
		r, err := s.application.ApplyAction(ctx, application.ApplyActionRequest{RequestID: w.RequestID, Host: w.Host, TaskID: w.TaskID, ExpectedRevision: w.Revision, ActionID: w.ActionID, ActionKind: w.ActionKind, ProcessDefinitionDigest: w.ProcessDefinitionDigest, RepositoryBindingDigest: w.RepositoryBindingDigest, Payload: w.Payload})
		if err != nil {
			return EncodeError(string(id), tool, err)
		}
		return EncodeSuccess(string(id), tool, projectTask(r.Task))
	case ToolCancelTask:
		var w cancelWire
		_ = decodeClosed(raw, &w)
		r, err := s.application.CancelTask(ctx, application.CancelTaskRequest{RequestID: id, Host: w.Host, TaskID: w.TaskID, ExpectedRevision: w.Revision, Reason: w.Reason})
		if err != nil {
			return EncodeError(string(id), tool, err)
		}
		return EncodeSuccess(string(id), tool, projectTask(r.Task))
