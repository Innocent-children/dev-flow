...
| `workflow` | `standard-development`, node rules, 31 transitions, TASKS planning and user confirmation, payloads, and invalidation |
...
The Windows process implementation hides console windows for read-only Git observation. The darwin implementation retains its native process behavior. Platform selection stays at the process boundary; Domain, Workflow, Application and Recovery keep shared task semantics. Platform validation is recorded in the [Windows report](../docs/WINDOWS-ADAPTATION_en.md).
