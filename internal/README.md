...
| `workflow` | `standard-development` 的节点规则、32 条转换、TASKS 计划与用户确认、提交字段和旧结果失效规则 |
| `application` | 创建、恢复、读取、提交、迁移、取消和放弃任务，并协调各组件 |
| `store` | SQLite 初始化、快照校验、CAS 并发更新、事件、工作树占用和只读预检查 |
| `repository` | 只读查询绑定工作目录的身份、历史、内容和任务改动，并限制查询范围；支持原目录分支模式和独立工作树 |
...
Windows 进程实现为只读 Git 观察隐藏控制台窗口，darwin 实现保留本机进程行为。平台选择位于进程边界，Domain、Workflow、Application 与 Recovery 共用任务语义。平台验证见[Windows 报告](../docs/WINDOWS-ADAPTATION.md)。
