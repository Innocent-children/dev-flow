...
func (s *stubControlCenterReader) GetTaskDetail(context.Context, application.GetControlCenterTaskRequest) (application.ControlCenterTaskDetail, error) {
	...
	return application.ControlCenterTaskDetail{Task: domain.ProcessTask{TaskID: "task", OriginHost: domain.HostCodex, Intent: domain.TaskIntent{Request: "Read a task", MethodProfile: domain.MethodPlain}, Process: domain.ProcessReference{ID: domain.ProcessStandardDevelopment, DefinitionDigest: digest}, CurrentNode: domain.NodeRequirements, PrimaryRepositoryKey: domain.DefaultPrimaryRepositoryKey, WorkspaceOrigin: domain.WorkspaceOrigin{Mode: domain.WorkspaceModeDedicatedWorktree, SourceType: "remote", RemoteName: "origin", BaseBranch: "main", BaseCommit: strings.Repeat("b", 40), TaskBranch: "feature/task", SourceRepositoryGroupDigest: digest, CanonicalWorktreeRoot: "/repo", WorktreeGitDirDigest: digest, ProvisioningReceiptID: "receipt"}, Repository: domain.RepositoryBinding{HistoryRelation: domain.RepositoryHistoryExact}, Revision: s.revision, CreatedAt: now, UpdatedAt: now}, ReadOnly: true}, nil
}
